...
code, head, html, redirect, log_ = hh.http(
    'https://passport.ustc.edu.cn/login', raw=raw3)

xsrf_token = re.search(r'Set-Cookie: (XSRF-TOKEN=.*?;)',
                       head).group(1).strip(';')
l_session = re.search(r'Set-Cookie: (laravel_session=.*?;)',
                      head).group(1).strip(';')
_token = re.search(r'<input type="hidden" name="_token" value="(.*?)">',
                   html).group(1)
now_address = re.search(
    r'<input type="radio" name="now_address" checked value="(.*?)">',
    html).group(1)
gps_now_address = re.search(
    r'<input type="hidden" name="gps_now_address" id="gps_now_address" value="(.*?)">',
    html).group(1)
now_province = re.search(
    r'<input type="hidden" id="now_province_hidden" value="(.*?)">',
    html).group(1)
gps_province = re.search(
    r'<input type="hidden" name="gps_province" id="gps_province" value="(.*?)">',
    html).group(1)
now_city = re.search(
    r'<input type="hidden" id="now_city_hidden" value="(.*?)">', html).group(1)
gps_city = re.search(
    r'<input type="hidden" name="gps_city" id="gps_city" value="(.*?)">',
    html).group(1)
now_detail = re.search(
    r'<textarea class="form-control" name="now_detail" id="now-detail" placeholder="请详细填写您所在的国家的位置，具体到国家、州、城市" rows="5">(.*?)</textarea>',
    html, re.DOTALL).group(1)
body_condition = re.search(
    r'<select name="body_condition" id="body-condition" rel="#body-detail" class="form-control selectpicker">(.*?)</select>',
    html, re.DOTALL).group(1)
body_condition = re.search(r'<option value="(\d*?)" selected>', body_condition,
                           re.DOTALL).group(1)
body_condition_detail = re.search(
    r'<textarea class="form-control" id="body-detail" name="body_condition_detail" rows="5">(.*?)</textarea>',
    html, re.DOTALL).group(1)
now_status = re.search(
    r'<select name="now_status" id="body-status" rel="#body-status-detail" class="form-control selectpicker">(.*?)</select>',
    html, re.DOTALL).group(1)
now_status = re.search(r'<option value="(\d*?)" selected>', now_status,
                       re.DOTALL).group(1)
now_status_detail = re.search(
    r'<textarea class="form-control" name="now_status_detail" id="body-status-detail" rows="5">(.*?)</textarea>',
    html, re.DOTALL).group(1)
has_fever = re.search(
    r'<input type="radio" name="has_fever" value="(.*?)" checked>',
    html).group(1)
last_touch_sars = re.search(
    r'<input type="radio" name="last_touch_sars" value="(.*?)" checked>',
    html).group(1)
last_touch_sars_date = re.search(
    r'<input type="text" id="last1-date" name="last_touch_sars_date" value="(.*?)" class="datetimepicker form-control" placeholder="请选择日期">',
    html).group(1)
last_touch_sars_detail = re.search(
    r'<textarea class="form-control" id="last1-detail" name="last_touch_sars_detail" rows="5">(.*?)</textarea>',
    html, re.DOTALL).group(1)
if now_province != '420000':
    last_touch_hubei = re.search(
        r'<input type="radio" name="last_touch_hubei" value="(.*?)" checked>',
        html).group(1)
    last_touch_hubei_date = re.search(
        r'<input type="text" id="last2-date" name="last_touch_hubei_date" class="datetimepicker form-control" value="(.*?)" placeholder="请选择日期">',
        html).group(1)
    last_touch_hubei_detail = re.search(
        r'<textarea class="form-control" id="last2-detail" name="last_touch_hubei_detail" rows="5">(.*?)</textarea>',
        html, re.DOTALL).group(1)
    last_cross_hubei = re.search(
        r'<input type="radio" name="last_cross_hubei" value="(.*?)" checked>',
        html).group(1)
    last_cross_hubei_date = re.search(
        r'<input type="text" id="last3-date" name="last_cross_hubei_date" class="datetimepicker form-control" value="(.*?)" placeholder="请选择日期">',
        html).group(1)
    last_cross_hubei_detail = re.search(
        r'<textarea class="form-control" name="last_cross_hubei_detail" id="last3-detail" rows="5">(.*?)</textarea>',
        html, re.DOTALL).group(1)
else:
    last_touch_hubei = ''
    last_cross_hubei = ''
...
return_dest = re.search(
    r'<input type="radio" name="return_dest" value="(.*?)" checked>',
    html).group(1)
return_dest_detail = re.search(
    r'<textarea class="form-control" name="return_dest_detail" id="mudidi-detail" rows="5">(.*?)</textarea>',
    html, re.DOTALL).group(1)
other_detail = re.search(
    r'<textarea class="form-control" name="other_detail" rows="5" placeholder="请输入其他情况说明">(.*?)</textarea>',
    html, re.DOTALL).group(1)
...
code, head, html, redirect, log_ = hh.http(
    'http://weixine.ustc.edu.cn/2020/daliy_report', raw=raw)

time_fuck = re.search(r'上次上报时间：(.*?)，', html, re.DOTALL).group(1)
print(time_fuck)
